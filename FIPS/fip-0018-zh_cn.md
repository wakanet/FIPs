...
该提案描述了将营销资产、文档、通信渠道和用户体验接口中现有的`矿工`参考重新命名为`存储提供商`的新定义。通过这项新提案，我们正在朝着更好地代表我们的社区合作伙伴的命名约定迈进，这些合作伙伴为Filecoin网络提供存储基础设施和服务。这一变化也将有助于突出Filecoin网络的有用性，并且是我们建议的一系列渐进变化的开始，以帮助网络创建新的叙事。我们欢迎对以下拟议变更的反馈和意见。
...
没有区块链是一样的。每一个区块链网络都具有独特的特征和激励机制，推动社区繁荣并实施长期可持续战略。
...
Filecoin的长期愿景始终是为人类最有价值的数据创建一个分散、高效和稳健的基础。为了支持这一愿景，我们依靠广泛的社区成员网络，向Filecoin网络提供并提交存储，目前称为Filecoin矿工。
...
随着Filecoin的快速存储增长和web3应用程序的采用，社区和扩展生态系统成员要求改变如何定位矿工的名称。

...
表示[Filecoin Plus](https://github.com/filecoin-project/FIPs/blob/master/FIPS/fip-0003.md)独立于内置市场参与者的数据上限分配。
向DataCap分配添加显式期限（持续时间）限制。
简化了质量调整后的功率计算，以消除在扇区寿命期间分散的功率。
支持独立于市场存储交易扩展数据上限分配条款。
将DataCap表示为可替换令牌。
这些变化为用户编程的市场参与者在未来代理Filecoin Plus验证交易提供了基础。

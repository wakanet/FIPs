...
Filecoin网络上的节点在两个tipset之间进行选择的一般过程是根据权重函数选择较重的tipset（参见[此处](https://spec.filecoin.io/#section-algorithms.expected_consensus.chain-weighting))。
...
两个tipset的权重相同并非不可能。当这种情况发生时，我们应该有一个简单的规则，选择其中一种tipset作为标准的“更重”，以便更快地达成共识。本FIP提出了这样一条规则。
